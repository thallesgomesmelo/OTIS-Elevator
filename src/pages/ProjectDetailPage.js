...
import { ArrowLeft, CheckCircle2, FileText, Calendar, User, DollarSign } from 'lucide-react';
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold">{new Date(project.startDate).toLocaleDateString('pt-BR')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold">{new Date(project.endDate).toLocaleDateString('pt-BR')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold line-clamp-1">{project.manager}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-shrink-0">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${
                        isComplete ? 'bg-green-100 dark:bg-green-950/30' : 'bg-muted'
                      }`}>
                        {isComplete ? <CheckCircle2 className="w-6 h-6 text-green-600" /> : stage.icon}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm font-medium">{stageData.complete}%</span>
                      </div>
                      
                      <Progress value={stageData.complete} className="h-2 mb-3" />
                      
                      <p className="text-sm text-muted-foreground mb-3">{stageData.notes}</p>
